...
datatable(ORVL, container = sketch, rownames = TRUE,  options = list(dom = 'tBp', buttons = c("csv","excel"), pageLength = 13)) %>% 
  formatStyle(
    0,
    target = "row",
...
```{r, echo=FALSE, message= FALSE, warning = FALSE}
datatable(ORPDC, rownames = TRUE,  options = list(dom = 'tBp', buttons = c("csv","excel"), pageLength = 13)) %>% 
  formatStyle(
    0,
    target = "row",
    fontWeight = styleEqual(c("Employment Status (4L)","Employment Status (2L)", "Gender", "Social priority", "Education Level (years)", "Addictions"), "bold"),
